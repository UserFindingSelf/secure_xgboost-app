...
if 'client_name' not in st.session_state or not st.session_state.client_name or not st.session_state.other_clients:
    ...
    if not st.session_state.key:
        st.info("If you don't select any option here then default keys and certificate will be selected")
        if st.session_state.option == 'Generate New':
            ...
            st.info('All files should have appropriate extension!')
            st.markdown("\
            ...
            if len(files) != 3:
                st.warning("Total 3 files should be present")
            else:
