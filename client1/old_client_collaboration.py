...
def run(channel_addr, sym_key_file, priv_key_file, cert_file):
    ...
    dtrain = xgb.DMatrix({username: HOME_DIR + f"{username}-train.enc?format=csv&label_column=0", "user2": HOME_DIR + "user2-train.enc?format=csv&label_column=0"})

    print("Creating test matrix")
    dtest1 = xgb.DMatrix({username: HOME_DIR + f"{username}-test.enc?format=csv&label_column=0"})
    dtest2 = xgb.DMatrix({"user2": HOME_DIR + "user2-test.enc?format=csv&label_column=0"})
